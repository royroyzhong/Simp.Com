...
function OrderTracking(props) {
    ...
    const columns = [
        ...
        {
            field: 'products', headerName: 'Products', width: 450,
            renderCell: (unprocessedProducts) => (
                <ul>
                    {unprocessedProducts.value.map((product, index) => (
                        <li key={index}> id: {product.id} name: {product.productName}</li>
                    ))}
                </ul>
            )
        },
    ...
    return (
        <Grid container rowSpacing={5} sx={{ marginLeft: "8vw" }} className="orderTreackingContent">
            <Grid item xs={10} className="orderTrackingHeader"> Unprocessed Orders</Grid>
            ...
            <Grid item xs={10} className="orderTrackingHeader"> In Shipment Orders</Grid>
            <Grid item xs={10} style={{ height: 400 }}>
                    <DataGrid
